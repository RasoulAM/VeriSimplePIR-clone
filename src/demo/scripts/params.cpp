...
int main() {
    ...
    uint64_t rows_db = std::getenv("ROWS_DB") ? std::stoull(std::getenv("ROWS_DB")) : (1ULL << 35);
    uint64_t cols_db = std::getenv("COLS_DB") ? std::stoull(std::getenv("COLS_DB")) : 1;
    N_d_params.push_back(std::vector<uint64_t>{rows_db, cols_db});
